...
### 🌐 Web 加载构筑站（实时加载主流 D4 构筑网站）
> 装备推荐 / 技能加点 / 巅峰加点三类指导全程通过 QWebEngineView 实时加载 d2core.com / maxroll / d4builds 等构筑站网页实现。
- 装备：实时加载 d2core.com 装备规划器 + 暗黑核数据库，按角色等级/赛季/词条自动匹配
- 技能：实时加载 maxroll / d4builds 主流构筑站，按主推 BD 自动定位加点配置
- 巅峰：实时加载 d2core.com 巅峰盘交互页，支持多盘垂直排列展示加点路径
...
├── graphical_overlay.py    # Web 加载构筑站降级方案（自绘 QPainter 面板，网页不可用时启用）
...
> BOSS 数据外置到 `boss_data.json`（赛季改版时可手动编辑），攻略菜单提供「🔄 刷新BOSS数据」热重载入口与「📦 导出BOSS数据模板」生成可编辑模板；BOSS 攻略加载支持游民星空攻略库 + 在线搜索（Bing + GLM 汇总）兜底。
